...
std::string Record::GetString(uint32_t field) const {
  ...
  return std::string(dbc_->string_block + offset);
}

void Record::SetStringRef(uint32_t field, const char* str) const {
  ValidateIndex(field - 1);
  stringref offset = GetUInt32(field);
  char* string_block_pointer = dbc_->string_block + offset;
  strcpy(string_block_pointer, str);
}

void Record::AddStringRef(uint32_t field, const char* str) const {
  ValidateIndex(field - 1);
  stringref offset = dbc_->header.string_block_size;
  SetUInt32(field, offset);

  dbc_->header.string_block_size += strlen(str) + 1;
